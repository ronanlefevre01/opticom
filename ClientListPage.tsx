...
export default function CampagnePage() {
  ...
  const [previewTitle, setPreviewTitle] = useState('Prévisualisation');
  ...
  // ⬇️ Preview using a modal (stable web/native)
  const handlePreview = (id: string) => {
    const msg = getMessageById(id);
    const all = [...campagnes, ...relanceLentilles, ...campagnesSaisonnieres];
    const found = all.find((c) => c.id === id);

    setPreviewTitle(found?.titre || 'Prévisualisation');
    setPreviewText(
      (msg || '')
        .replace('{prenom}', 'Jean')
        .replace('{nom}', 'Dupont')
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.previewCard}>
            ...
            <TouchableOpacity
              style={[styles.button, { marginTop: 14, backgroundColor: '#555' }]}
...
const styles = StyleSheet.create({
  ...
  previewCard: { backgroundColor: '#222', padding: 22, borderRadius: 12, width: '85%' },
  previewTitle: { color: '#fff', fontWeight: '700', fontSize: 16, marginBottom: 8 },
  previewBody: { color: '#ddd', lineHeight: 22 },
